...
pub async fn from_file(
    ...
    let body = hal.tr("Restore backup?");
    hal.ui()
    ...
    let (data, metadata) = match crate::backup::load(hal, &request.id).await {
        Ok(d) => d,
        Err(_) => {
            let status = hal.tr("Could not\nrestore backup");
            hal.ui().status(&status, false).await;
            return Err(Error::Generic);
        }
    };

    let body = hal.tr_format("Name: {}. ID: {}", &[&metadata.name, &request.id]);
    hal.ui()
        .confirm(&ConfirmParams {
            body: &body,
            scrollable: true,
    ...
    if let Err(err) = crate::keystore::encrypt_and_store_seed(hal, seed, &password).await {
        ...
        let mut status = hal.tr("Could not\nrestore backup").into_owned();
        status.push('\n');
...
pub async fn from_mnemonic(
    ...
    let mnemonic = crate::workflow::mnemonic::get(hal.ui(), language).await?;
    let seed = match crate::bip39::mnemonic_to_seed(&mnemonic) {
        Ok(seed) => seed,
        Err(()) => {
            let status = hal.tr("Recovery words\ninvalid");
            hal.ui().status(&status, false).await;
            return Err(Error::Generic);
        }
    };
