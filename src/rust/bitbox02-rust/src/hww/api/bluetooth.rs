...
async fn process_upgrade(
    ...
    let title = hal.tr("Bluetooth");
    let body = hal.tr("Upgrade\nfirmware?");
    hal.ui()
    ...
    let progress_title = hal.tr("Upgrading...");
    ...
    if response.is_ok() {
        ...
        let status = hal.tr("Upgrade failed");
...
async fn process_toggle_enabled(hal: &mut impl crate::hal::Hal) -> Result<Response, Error> {
    ...
    let body = if enabled {
        hal.tr("Disable Bluetooth?")
    } else {
        hal.tr("Enable Bluetooth?")
    };
