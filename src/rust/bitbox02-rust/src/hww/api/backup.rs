...
pub async fn check(
    ...
    if seed.as_slice() != backup_data.get_seed() {
        if !silent {
            let status = hal.tr("Backup missing\nor invalid");
            hal.ui().status(&status, false).await;
        }
        return Err(Error::Generic);
    }
    if !silent {
        let title = hal.tr("Name?");
        hal.ui()
        ...
        let status = hal.tr("Backup valid");
...
pub async fn create(
    ...
    let seed = if is_initialized {
        let title = hal.tr("Unlock device");
    ...
    match backup::create(hal, &seed, &device_name, timestamp, seed_birthdate).await {
        Ok(()) => {
            ...
            let status = hal.tr("Backup created");
            hal.ui().status(&status, true).await;
