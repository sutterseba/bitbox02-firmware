...
impl ParsedPolicy<'_> {
    ...
    pub async fn confirm(
        ...
        let policy_title = hal.tr("Policy");
        hal.ui()
            .confirm(&ConfirmParams {
                title: &policy_title,
                body: &policy.policy,
        ...
        for (i, key) in policy.keys.iter().enumerate() {
            ...
            if self.is_our_key[i] {
                key_str = hal.tr_format("This device: {}", &[&key_str])
            } else if Some(i) == taproot_unspendable_internal_key_index {
                key_str = hal.tr_format("Provably unspendable: {}", &[&key_str])
            }
            let key_title =
                hal.tr_format("Key {}/{}", &[&(i + 1).to_string(), &num_keys.to_string()]);
            hal.ui()
                .confirm(&ConfirmParams {
                    title: &key_title,
                    body: key_str.as_str(),
