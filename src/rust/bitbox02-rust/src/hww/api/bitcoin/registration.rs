...
async fn get_name(
    ...
    let name = if request.name.is_empty() {
        let title = hal.tr("Register");
        let body = hal.tr("Please name this\naccount");
        hal.ui()
        ...
        let title = hal.tr("Enter account name");
        let name = hal
            .ui()
            .enter_string(
...
pub async fn process_register_script_config(
    ...
    match request.registration.as_ref() {
        Some(pb::BtcScriptConfigRegistration {
            ...
            let status = hal.tr("Multisig account\nregistered");
