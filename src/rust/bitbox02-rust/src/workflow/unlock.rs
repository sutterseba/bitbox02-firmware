...
async fn confirm_mnemonic_passphrase(
    ...
    let body = hal.tr("You will be asked to\nvisually confirm your\npassphrase now.");
...
pub async fn unlock_bip39<H: crate::hal::Hal>(
    ...
    if hal.memory().is_mnemonic_passphrase_enabled() {
        ...
        loop {
            let title = hal.tr("Optional passphrase");
            ...
            let status = hal.tr("Please try again");
            hal.ui().status(&status, false).await;
        }
    }
...
pub async fn unlock(hal: &mut impl crate::hal::Hal) -> Result<(), ()> {
    ...
    loop {
        let title = hal.tr("Enter password");
